...
parser.add_argument('-ds','--death_size', type=float, default = 0.03, help='Percent chance that a death will occur, default value = 0.03')
...
def first_function(N,M,L,max_iter,number_of_simulations,population_density_value,step_value,percent_value,death_percent):
	...
	final_ts_recovered = []
	final_n_recovered = []
	average_number_of_days_sick = []
	sick_list_without_zeros = []
	...
	for i in range (0,number_of_simulations):
		...
		sickness_lifespan = np.zeros(M)
		initial_sickness_lifespan = np.zeros(M)
		number_of_days_sick = np.zeros(M)
		ts_sick = []
		ts_population = []
		ts_recovered = []
		...
		for j in range(M):
			...
			random_sickness_lifespan = np.random.randint(14,23)  #Picking a random value for number of days the walker is sick
			#print(random_sickness_lifespan)
			sickness_lifespan[j] = random_sickness_lifespan
			initial_sickness_lifespan[j] = random_sickness_lifespan

		initial_random_walker = np.random.randint(0,M-1)  #Picking a random host for virus
		infect[initial_random_walker] = 1
		n_sick,n_dead,n_recovered,iterate = 1,0,0,0  #counters
		remaining_population = M

		while (n_sick > 0) and (iterate < max_iter):  #temporal iteration
			for j in range (0,M):  #loop over all walkers
				if infect[j] < 3:  #this walker is still alive
				...
				if infect[j] == 1:  #this walker is sick
					sickness_lifespan[j] -= 1
					#Counts the number of days each walker is sick (Random number of days between a range is chosen at the beginning of the simulation), if the walker dies within that time the number of sick days is smaller than the range
					number_of_days_sick[j] += 1

					if sickness_lifespan[j] <= 0:
						infect[j] = 2
						n_sick -= 1
						n_recovered += 1
					
					random_death_chance = random.randint(1, 100)/100
					if random_death_chance <= death_percent:
						infect[j] = 3
						n_sick -= 1
						n_dead += 1
			...
			ts_recovered.append(n_recovered)
			iterate += 1

			#print("Current iteration = {0}, healthy population = {1}, sick = {2}, dead = {3}, recovered = {4}.".format(iterate,remaining_population,n_sick,n_dead,n_recovered))
		final_ts_populations.append(ts_population)
		#print(final_ts_populations)
		final_ts_sick.append(ts_sick)
		final_n_recovered.append(n_recovered)
		print(number_of_days_sick)
		sick_list_without_zeros = [m for m in number_of_days_sick if m != 0]
		average_number_of_days_sick.append(Average(sick_list_without_zeros))
		print(average_number_of_days_sick)
	...
	average_of_all_recovered = Average(final_n_recovered)

	average_of_sick_all_sims = Average(average_number_of_days_sick)
	#final_average_of_sick_all_sims = Average(average_of_sick_all_sims)
	#print(final_ts_populations)
	print("---------------------------------------------------")
	print("Average for {0} simulations, death chance {1}, average number of recovered walkers = {2} / average number of days sick = {3}.".format(number_of_simulations,death_percent,average_of_all_recovered,average_of_sick_all_sims))
...
if args.function == 'function_1':
	first_function(N,M,L,max_iter,number_of_simulations,population_density_value,step_value,percent_value,death_percent)
